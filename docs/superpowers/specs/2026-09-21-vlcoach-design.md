...
*Amended 2026-09-24:* the owner lifted the permanent ban. `StealthyFetcher` and bypass logic are now allowed by `AGENTS.md`, but v0.2 still ships `DynamicFetcher` only. Adopting any of them needs an observed block first and its own recorded design change. Fail closed stays.
